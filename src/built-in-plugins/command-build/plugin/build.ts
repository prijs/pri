...
import { PublishOption } from '../../command-publish/plugin/interface';
...
export const buildComponent = async (packageInfo: PackageInfo = null, options: PublishOption = null) => {
  // FIXME:
  ...
  await spinner(`build source files ${(packageInfo && packageInfo.name) || ''} \n`, async () => {
    await tsPlusBabel(false, false, packageInfo, options);
  });
