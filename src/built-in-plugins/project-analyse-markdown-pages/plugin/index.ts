...
pri.project.onCreateEntry((analyseInfo: IResult, entry) => {
  ...
  entry.pipeAppComponent(async entryComponent => {
    return `
        ${(await Promise.all(
          analyseInfo.projectAnalyseMarkdownPages.pages.map(async page => {
            ...
            if (
              ...
              const { markdown } = await import('./markdown-helper');
  ...
  entry.pipeAppRoutes(async renderRoutes => {
    return `
        ${(await Promise.all(
          analyseInfo.projectAnalyseMarkdownPages.pages.map(async page => {
            return `
              <${await entry.pipe.get('markdownRoute', 'Route')} exact path="${page.routerPath}" component={${
              page.componentName
            }} />\n
            `;
          })
